...
public class WaveletFilter< T extends RealType< T >, K extends IterableInterval< T > & RandomAccessibleInterval< T >> implements UnaryOperation< K, K >
{
	...
	public K compute( final K src, final K res )
	{
...

		{
			...
			{
				if ( m_tempCursor.next().getRealDouble() < m_lambda && m_tempCursor.next().getRealDouble() > -m_lambda )
				{
	...
	public WaveletFilter( final ExecutorService executor, final double lambda )
	{
		m_executor = executor;
		m_lambda = lambda;
	}
