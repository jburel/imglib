...
public final class KNIPLibHistogram
{
	...
	public KNIPLibHistogram( final int[] hist, final double min, final double max )
	{
		m_bins = hist.length;
		m_hist = hist;
		m_min = min;
	...
	public KNIPLibHistogram( final int bins, final double min, final double max )
	{
		this( new int[ bins ], min, max );
	}

	public < T extends RealType< T >> KNIPLibHistogram( final int bins, final T type )
	{
