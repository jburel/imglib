...
public final class MakeHistogram< T extends RealType< T >> implements UnaryOutputOperation< Iterable< T >, KNIPLibHistogram >
{
	...
	{
		return m_numBins <= 0 ? new KNIPLibHistogram( op.iterator().next().createVariable() ) : new KNIPLibHistogram( m_numBins, op.iterator().next().createVariable() );
	}
