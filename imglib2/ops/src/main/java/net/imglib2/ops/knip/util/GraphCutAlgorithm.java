...
{
	/**
	 * The two possible segments, represented as special terminal nodes in the
	 * graph.
	 */
	 ...
	 * @param numEdges
	 *            The number of edges that you can add. A directed edge and its
	 *            counterpart (i.e., the directed edge in the other direction)
	 *            count as one edge.
	 */
	 ...
	 * @param source
	 *            The affinity of this node to the foreground (i.e., source)
	 * @param sink
	 *            The affinity of this node to the background (i.e., sink)
	 */
	 ...
	 *            The second node.
	 * @param weight1to2
	 *            The weight (i.e., the cost) of the directed edge from node1 to
	 *            node2.
	 * @param weight2to1
	 *            The weight (i.e., the cost) of the directed edge from node2 to
	 *            node1.
	 */
	 ...
	 * @param changedNodes
	 *            list of nodes that potentially changed their segmentation
	 *            compared to a previous call, can be set to <tt>null</tt>
	 */
			...
			if ( !activeNode.isInSink() )
			{
				...
				for ( edge = activeNode.getFirstOutgoing(); edge != null; edge = edge.getNext() )
				{
					if ( edge.getResidualCapacity() != 0 )
					{
						...
						else if ( headNode.getTimestamp() <= activeNode.getTimestamp() && headNode.getDistance() > activeNode.getDistance() )
						{
							// node is not free and
							// belongs to our tree -
							// try to
	...
	public float getEdgeWeight( int nodeID1, int nodeID2 )
	{
		...
		{
			e = edges[ i ];
			if ( e.getHead().equals( n1 ) && e.getSister().getHead().equals( n2 ) ) { return e.getResidualCapacity(); }
		}
		return 0;
