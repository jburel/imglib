...
/**
 * Class utilizes ImageJ1 implementation
 * 
 * @author dietzc, hornm, schoenenbergerf University of Konstanz
 */
public class FindThreshold< T extends RealType< T >> implements UnaryOperation< KNIPLibHistogram, DoubleType >
{
	...
	static int MinErrorI( final int[] data )
	{
		...
		while ( threshold != Tprev )
		{
			...
			nu = ( B( data, MAXVALUE ) - B( data, threshold ) ) / ( A( data, MAXVALUE ) - A( data, threshold ) );
			p = A( data, threshold ) / A( data, MAXVALUE );
			q = ( A( data, MAXVALUE ) - A( data, threshold ) ) / A( data, MAXVALUE );
			sigma2 = C( data, threshold ) / A( data, threshold ) - ( mu * mu );
			tau2 = ( C( data, MAXVALUE ) - C( data, threshold ) ) / ( A( data, MAXVALUE ) - A( data, threshold ) ) - ( nu * nu );
	...
	static int RenyiEntropy( final int[] data )
	{
		...
		optThreshold = ( int ) ( tStar1 * ( P1[ tStar1 ] + 0.25 * omega * beta1 ) + 0.25 * tStar2 * omega * beta2 + tStar3 * ( P2[ tStar3 ] + 0.25 * omega * beta3 ) );
