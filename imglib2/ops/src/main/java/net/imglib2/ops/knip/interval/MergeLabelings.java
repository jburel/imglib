...
public final class MergeLabelings< L extends Comparable< L >> implements UnaryOutputOperation< Labeling< L >[], NativeImgLabeling< L, ? extends IntegerType< ? >>>
{
	 ...
	 * offset
	 */
	private < T extends Type< T >> void writeInterval( RandomAccess< T > resAccess, IterableInterval< T > interval, long[] offset )
	{
		...
		while ( localizingCursor.hasNext() )
		{
			...
			for ( int d = 0; d < interval.numDimensions(); d++ )
			{
				resAccess.setPosition( localizingCursor.getIntPosition( d ) - offset[ d ], d );
			}
