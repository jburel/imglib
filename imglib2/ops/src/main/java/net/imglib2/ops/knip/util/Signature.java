...
public class Signature
{
	 ...
	 */
	public Signature( final IterableInterval< ? extends RealType< ? >> interval, int maxVariance )
	{
	 ...
	 *            (corresponds to the radius)
	 */
	public Signature( final IterableInterval< ? extends RealType< ? >> interval, final int historyLength, int maxVariance )
	{
	...
	/**
	 * Retrieves a signature from a bit mask. The first occurrence of the change
	 * from ON_VALUE to OFF_VALUE will used for the individual signature
	 * positions.
	 * 
	 * @param mask
	 * @param maskPosition
	...
	public Signature( final Img< BitType > mask, long[] maskPosition, final int signatureLength )
	{
		...
		RandomAccess< BitType > ra = Views.extend( mask, new OutOfBoundsConstantValueFactory< BitType, Img< BitType >>( new BitType( false ) ) ).randomAccess();
		int rimWidth = ( int ) Math.round( Math.sqrt( Math.pow( mask.dimension( 0 ), 2 ) + Math.pow( mask.dimension( 1 ), 2 ) ) );
		m_width = rimWidth;
		...
		for ( double a = 0; Math.round( a / step ) < signatureLength; a += step )
		{
			for ( int r = 0; r < rimWidth; r++ )
			{
				...
				if ( tmpx >= mask.dimension( 0 ) - 1 || tmpy >= mask.dimension( 1 ) - 1 || tmpx <= 0 || tmpy <= 0 )
				{
	...
	public Signature( final IterableRegionOfInterest roi, long[] maskPosition, final int signatureLength )
	{
		...
		IterableInterval< BitType > ii = roi.getIterableIntervalOverROI( new ConstantRandomAccessible< BitType >( new BitType(), roi.numDimensions() ) );
		...
		int rimWidth = ( int ) Math.round( Math.sqrt( Math.pow( ii.dimension( 0 ) / 2, 2 ) + Math.pow( ii.dimension( 1 ) / 2, 2 ) ) );
	 ...
	 * 
	 * @param sign
	 *            the 'radial' positions for each angle (polar space). A copy
	 *            will be made.
	 * @param width
	...
	/**
	 * The position (corresponds to the radius) of given index (corresponds to
	 * the angle).
	 * 
	 * @param index
	 * @return
	...
	public void centralize()
	{
		...
		for ( int i = 0; i < m_sign.length; i++ )
		{
			...
			int test = ( int ) Math.round( ang / ( 2 * Math.PI ) * ( ( double ) m_sign.length - 1 ) );
	...
	{
		if ( s.length() != m_sign.length ) { return false; }
		int diff = 0;
		for ( int i = 0; i < m_sign.length; i++ )
		{
	...
	/**
	 * Creates an image (width x length) from the signature, where all pixels
	 * from the signature are on, the rest off.
	 * 
	 * @return the image
	 */
	public Img< BitType > createImage()
	{
		Img< BitType > res = new ArrayImgFactory< BitType >().create( new long[] { m_width, m_length }, new BitType() );
		RandomAccess< BitType > c = Views.extend( res, new OutOfBoundsConstantValueFactory< BitType, Img< BitType >>( new BitType( false ) ) ).randomAccess();
	...
	public Point getCartCoords( final int x, final int y )
	{
		...
		int tmpx = ( int ) Math.round( x * Math.cos( ( ( double ) y / ( double ) m_sign.length ) * 2 * Math.PI ) ) + ( int ) m_center[ 0 ];
		int tmpy = -( int ) Math.round( x * Math.sin( ( ( double ) y / ( double ) m_sign.length ) * 2 * Math.PI ) ) + ( int ) m_center[ 1 ];
	...
	/**
	 * Smoothes the signature by cutting the desired frequencies. This operation
	 * requires the signature to have a length of power of 2! If not, an
	...
	{
		double[][] weights = new double[ ( int ) interval.dimension( 1 ) ][ ( int ) interval.dimension( 0 ) ];
	...
	/*
	 * Method, which helps to extract the signature from a polar image with a
	 * arbitrary depth m_historyLength
	 */

	private int[] extractLine( double[][] weights, int maxLineVariance, int historyLength )
	{
	...
	/*
	 * Object represents a path though the plane of a polar image of a specific
	 * length
	 */
	private static class Path implements Comparable< Path >, Cloneable
	{
		...
		void addPos( int index, final int pos )
		{
			...
			{
				m_globalScore -= m_weights[ ( index - m_weights.length ) % m_weights.length ][ m_pos[ ( index - m_weights.length ) % m_weights.length ] ];
			}

			if ( index >= m_h )
			{
				m_score -= m_weights[ ( index * 10 - m_h ) % m_weights.length ][ m_pos[ ( index * 10 - m_h ) % m_weights.length ] ];
			}
