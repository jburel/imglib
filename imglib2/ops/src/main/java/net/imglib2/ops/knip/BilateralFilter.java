...
public class BilateralFilter< T extends RealType< T >, K extends RandomAccessibleInterval< T > & IterableInterval< T >> implements UnaryOperation< K, K >
{
	...
	public BilateralFilter( double sigma_r, double sigma_s, int radius, int[] selectedDims )
	{
		m_sigma_r = sigma_r;
		m_sigma_s = sigma_s;
		m_radius = radius;
	...
	public K compute( K srcIn, K res )
	{
		...
		while ( cp.hasNext() )
		{
			...
			cp.localize( mi );
			cp.localize( ma );
			mi[ m_selectedDims[ 0 ] ] = Math.max( 0, mi[ m_selectedDims[ 0 ] ] - m_radius );
			mi[ m_selectedDims[ 1 ] ] = Math.max( 0, mi[ m_selectedDims[ 1 ] ] - m_radius );
			ma[ m_selectedDims[ 0 ] ] = Math.min( mma1, mi[ m_selectedDims[ 0 ] ] + m_radius );
			ma[ m_selectedDims[ 1 ] ] = Math.min( mma2, mi[ m_selectedDims[ 1 ] ] + m_radius );
			Interval in = new FinalInterval( mi, ma );
			si = new IterableRandomAccessibleInterval< T >( SubImg.getView( srcIn, in, false ) );
			...
			while ( cq.hasNext() )
			{
				...
				cq.localize( q );
				// d = 0.0;
				d = ( p[ m_selectedDims[ 0 ] ] - q[ m_selectedDims[ 0 ] ] - mi[ m_selectedDims[ 0 ] ] ) * ( p[ m_selectedDims[ 0 ] ] - q[ m_selectedDims[ 0 ] ] - mi[ m_selectedDims[ 0 ] ] ) + ( p[ m_selectedDims[ 1 ] ] - q[ m_selectedDims[ 1 ] ] - mi[ m_selectedDims[ 1 ] ] ) * ( p[ m_selectedDims[ 1 ] ] - q[ m_selectedDims[ 1 ] ] - mi[ m_selectedDims[ 1 ] ] );
	...
	{
		return new BilateralFilter< T, K >( m_sigma_r, m_sigma_s, m_radius, m_selectedDims );
	}
}
