...
/**
 * n-Dimension Bresenham line ({@linkplain http://en.wikipedia.org/wiki/Bresenham%27s_line_algorithm}):
 * an iterator that iterates all the pixels between two points. 
 * <p>
 * This implementation uses eager initialization: all the coordinates along the line are 
 * calculated at once when the {@link #reset(Localizable, Localizable)} method is called.
 * <p>
 * Adapted from the <code>Bresenham3D</code> class in VIB-lib by Johannes Schindelin.
 * 
...
public class BresenhamLine<T> implements Cursor<T> {
	...
	public void localize(long[] position) {
		position = Arrays.copyOf(current, current.length);
	}
